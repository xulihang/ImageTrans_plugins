...
Sub batchTranslate(sourceList As List, sourceLang As String, targetLang As String,preferencesMap As Map,terms As Map) As ResumableSub
	...
	Dim apikey As String = getMap("ollama",getMap("mt",preferencesMap)).Get("key")
	Dim host As String = getMap("ollama",getMap("mt",preferencesMap)).GetDefault("host","http://localhost:11434")
	
	Dim prompt As String
	If terms.Size>0 Then
		prompt = getMap("ollama",getMap("mt",preferencesMap)).GetDefault("batch_prompt_with_term",defaultBatchPromptWithTerm)
	Else
...
Sub translate(source As String,sourceLang As String,targetLang As String,preferencesMap As Map,terms As Map) As ResumableSub
	...
	Dim key As String = getMap("ollama",getMap("mt",preferencesMap)).Get("key")
	Dim prompt As String
	If terms.Size>0 Then
		prompt = getMap("ollama",getMap("mt",preferencesMap)).GetDefault("prompt_with_term",defaultPromptWithTerm)
	Else
		prompt = getMap("ollama",getMap("mt",preferencesMap)).GetDefault("prompt",defaultPrompt)
	End If
