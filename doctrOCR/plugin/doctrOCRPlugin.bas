...
public Sub Run(Tag As String, Params As Map) As ResumableSub
	...
	Select Tag
		...
		Case "getTextWithLocation"
			...
			Return regions
		Case "getSetupParams"
			Dim o As Object = CreateMap("readme":"https://github.com/xulihang/ImageTrans_plugins/tree/master/doctrOCR")
			Return o
		Case "getDefaultParamValues"
			Return CreateMap("url":"http://127.0.0.1:8189/ocr")
...
Sub ocr(img As B4XBitmap, lang As String) As ResumableSub
	...
	job.PostMultipart(url,CreateMap("lang":lang,"engine":"doctr"), Array(fd))
...
Sub getUrl As String
	Dim url As String = "http://127.0.0.1:8189/ocr"
	If File.Exists(File.DirApp,"preferences.conf") Then
		Try
			Dim preferencesMap As Map = readJsonAsMap(File.ReadString(File.DirApp,"preferences.conf"))
			url=getMap("doctr",getMap("api",preferencesMap)).GetDefault("url",url)
		Catch
			Log(LastException)
		End Try
	End If
