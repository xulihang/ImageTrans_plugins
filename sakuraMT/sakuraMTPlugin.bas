...
Sub Class_Globals
	Private fx As JFX
	Private defaultPrompt As String = $"Translate the following into Chinese: {source}"$
	...
	Private defaultBatchPrompt As String = $"Translate the following into Chinese: {source}"$
	Private defaultBatchPromptWithTerm As String = $"Translate the following into Chinese: {source} You should use the terms defined in JSON: {term}."$
	Private defaultHost As String = "http://127.0.0.1:8080/v1"
	Private defaultModel As String = "onekuma/sakura-13b-lnovel-v0.9b-q2_k"
End Sub
...
Sub batchTranslate(sourceList As List, sourceLang As String, targetLang As String,preferencesMap As Map,terms As Map) As ResumableSub
	...
	Dim apikey As String = getMap("sakura",getMap("mt",preferencesMap)).Get("key")
	Dim host As String = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("host",defaultHost)
	
	Dim prompt As String
	If terms.Size>0 Then
		prompt = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("batch_prompt_with_term",defaultBatchPromptWithTerm)
	Else
		prompt = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("batch_prompt",defaultBatchPrompt)
	End If
	
	Dim model As String = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("model",defaultModel)
...
Sub translate(source As String,sourceLang As String,targetLang As String,preferencesMap As Map,terms As Map) As ResumableSub
	...
	Dim key As String = getMap("sakura",getMap("mt",preferencesMap)).Get("key")
	Dim prompt As String
	If terms.Size>0 Then
		prompt = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("prompt_with_term",defaultPromptWithTerm)
	Else
		prompt = getMap("sakura",getMap("mt",preferencesMap)).GetDefault("prompt",defaultPrompt)
	End If
