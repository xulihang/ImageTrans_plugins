...
Sub batchTranslate(sourceList As List,sourceLang As String,targetLang As String,preferencesMap As Map) As ResumableSub
	...
	Dim key As String
	key=getMap("lingvanex",getMap("api",preferencesMap)).GetDefault("key","")
	If key="" Then
		Return targetList
	End If
	params="key="&key
	...
	job.PostString("https://api-gl.lingvanex.com/language/translate/v2?"&params,jsonG.ToString)
...
Sub translate(source As String,sourceLang As String,targetLang As String,preferencesMap As Map) As ResumableSub
	...
	End If
	params="key="&key& _
	"&q="&su.EncodeUrl(source,"UTF-8")&"&model=&format=text&source="&sourceLang&"&target="&targetLang
	job.PostString("https://api-gl.lingvanex.com/language/translate/v2",params)
	wait For (job) JobDone(job As HttpJob)
	If job.Success Then
		Try
			Dim result,data As Map
