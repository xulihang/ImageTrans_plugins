...
public Sub Run(Tag As String, Params As Map) As ResumableSub
	...
	Select Tag
		...
		Case "inpaint"
			...
			Return result
		Case "getSetupParams"
			Dim o As Object = CreateMap("readme":"https://github.com/xulihang/ImageTrans_plugins/tree/master/AOTInpaint")
			Return o
...
Private Sub LoadIfNeeded As ResumableSub
	If engine.IsInitialized = False Then
		engine.Initialize(File.Combine(File.DirApp,"inpainting_aot.onnx"))
	End If
	Return True
End Sub
...
Sub inpaint(origin As B4XBitmap,mask As B4XBitmap) As ResumableSub
	If ONNXExists Then
		Wait For (LoadIfNeeded) complete (done As Object)
...
Private Sub ONNXExists As Boolean
	If File.Exists(File.DirApp,"inpainting_aot.onnx") Then
		Return True
	End If
	Return False
End Sub
